...
export function createDefaulAIClient(overrides: AIClientOptions = {}) {
    ...
    } else if (overrides.provider == "anthropic") {
        baseURL = process.env.ANTHROPIC_BASE_URL || "https://api.moonshot.cn/v1";
    }

    let model =  process.env.MODEL_ID || "kimi-k2-0711-preview";
    return new BasicAIClient({ apiKey, baseURL, model, provider: overrides.provider || 'openai', ...overrides });
}